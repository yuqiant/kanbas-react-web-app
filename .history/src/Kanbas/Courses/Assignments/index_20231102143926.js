...
function Assignments() {
    ...
    const handleDeleteAssignment = (assignmentId) => {
        setDeleteAssignmentId(assignmentId); // 设置要删除的作业的ID
    };

    const handleConfirmDelete = () => {
        if (deleteAssignmentId) {
            dispatch(deleteAssignment(deleteAssignmentId)); // 执行删除操作
            setDeleteAssignmentId(null); // 清除要删除的作业的ID
        }
    };

    const handleCancelDelete = () => {
        setDeleteAssignmentId(null); // 取消删除，清除要删除的作业的ID
    };
